...
JustAfew:
	...
	// We may need to do some alignment work up front, and at the end, so that the
	// main loop is aligned and only has to worry about 8 byte at a time.
...
{
	int PreLength, InLength, PostLength;
	unsigned char* pb = (unsigned char*)p;
	CRC32_t ulCrc = *pulCRC;
	// Break the computation up into.
	PreLength = ((int)p) & 3;
	len -= PreLength;
	InLength = len / 4;
	PostLength = len % 4;
	// Do part of buffer that isn't aligned to a long word boundary.
...
// YWB:  5/18
// BOOL MD5_Hash_File(unsigned short *crcvalue, char *pszFileName)
		...
		while (*cur)
		{
			...
			while (nSize > 0)
			{
				if (nSize > sizeof(chunk))
					nBytesRead = (int)fread(chunk, 1, sizeof(chunk), fp);
				else
					nBytesRead = (int)fread(chunk, 1, nSize, fp);
...
int MD5_Hash_CachedFile( unsigned char digest[16], unsigned char *pCache, int nFileSize, int bSeed, unsigned int seed[4] )
{
	...
	MD5Update( &ctx, pCache, nFileSize );
	MD5Final( digest, &ctx );
	return 1;
}
