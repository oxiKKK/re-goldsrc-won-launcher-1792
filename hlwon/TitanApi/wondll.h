...
// NOTE(ox): the surviving wondll.h predates the Profile service and declares
// none of it, so add the handle here in the style of the others.
typedef struct unnamed_HWONPROFILE { int unused; }* HWONPROFILE;
...
// Profile API.
//
// The wondll.h in the surviving SDK predates the Profile service: it declares
// none of these, so their definitions in Profile/ProfileAPI.cpp would take C++
// linkage and the undecorated names the 2001 DLL exports would not resolve.
// Signatures below are transcribed from those definitions, which are the only
// surviving record of them.
// ---------------------------------------------------------------------------

void WONProfileCloseHandle(HWONPROFILE hProfile);
WONError WONProfileCreateAccount(HWONAUTH hAuth, WON_CONST WONIPAddress* profileSrvrs, unsigned int numSrvrs, HWONPROFILE hProfile, long sourceSeq, BOOL wantsPromos, long timeout);
void WONProfileCreateAccountAsync(HWONAUTH hAuth, WON_CONST WONIPAddress* profileSrvrs, unsigned int numSrvrs, HWONPROFILE hProfile, long sourceSeq, BOOL wantsPromos, long timeout, HWONCOMPLETION hCompletion);
WON_CONST void* WONProfileGet(HWONPROFILE hProfile, unsigned short index, unsigned short* size);
WONError WONProfileGetAccount(HWONAUTH hAuth, WON_CONST WONIPAddress* profileSrvrs, unsigned int numSrvrs, HWONPROFILE hProfile, long timeout);
void WONProfileGetAccountAsync(HWONAUTH hAuth, WON_CONST WONIPAddress* profileSrvrs, unsigned int numSrvrs, HWONPROFILE hProfile, long timeout, HWONCOMPLETION hCompletion);
...
WONError WONProfileUpdateAccount(HWONAUTH hAuth, WON_CONST WONIPAddress* profileSrvrs, unsigned int numSrvrs, HWONPROFILE hProfile, long timeout);
void WONProfileUpdateAccountAsync(HWONAUTH hAuth, WON_CONST WONIPAddress* profileSrvrs, unsigned int numSrvrs, HWONPROFILE hProfile, long timeout, HWONCOMPLETION hCompletion);
