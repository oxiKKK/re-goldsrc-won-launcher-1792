...
// Export or import to/from DLL.  WON_STATIC_LINK (LAUNCHER_STATIC) folds WONCrypt
// straight into hl.exe, so there is no DLL boundary left to describe -- DLL_OPER
// collapses to nothing on both the library and the consumer side.
#if defined(WON_STATIC_LINK)
#define DLL_OPER
#elif defined(_WON_SOURCE_)
