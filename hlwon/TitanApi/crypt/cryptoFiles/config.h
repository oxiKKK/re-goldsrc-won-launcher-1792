...
// MSVC 6.0's <algorithm> does not provide usable std::min/std::max here, so define
// STDMIN/STDMAX as portable inline templates rather than aliasing std::min/std::max.
// Every Crypto++ call site uses STDMIN/STDMAX, so this is the single point of fix
// (works identically on MSVC 6.0 and modern toolsets).
template <class T> inline const T& STDMIN(const T& a, const T& b) { return b < a ? b : a; }
template <class T> inline const T& STDMAX(const T& a, const T& b) { return a < b ? b : a; }
