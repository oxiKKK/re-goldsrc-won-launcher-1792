...
public:
	...
	public:
		...
		public:
			...
			int operator==(const unsigned char* nodeNum)	{ return memcmp(val, nodeNum, 6) == 0; }
			int operator==(const char* nodeNum)				{ return memcmp(val, nodeNum, 6) == 0; }
			int operator==(const NodeNum& nodeNum)			{ return memcmp(val, &(nodeNum.val), 6) == 0; }
			int operator!=(const unsigned char* nodeNum)	{ return memcmp(val, nodeNum, 6) != 0; }
			int operator!=(const char* nodeNum)				{ return memcmp(val, nodeNum, 6) != 0; }
			int operator!=(const NodeNum& nodeNum)			{ return memcmp(val, &(nodeNum.val), 6) != 0; }
