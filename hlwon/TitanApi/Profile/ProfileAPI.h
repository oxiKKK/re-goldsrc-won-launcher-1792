...
// The Profile class below indexes WONMsg::PROFILE_*; ProfileAPI.cpp includes this
// only after ProfileAPI.h, which is part of why the class was left disabled.
...
// NOTE(ox): the leaked drop has this whole class commented out, but the 2001
// WONDLL.dll exports the Profile API that depends on it, so it is enabled here.
namespace WONAPI {
...
inline Error UpdateProfileEx(Identity& ident, const IPSocket::Address& profileSrvr,
							 ...
							 void (*f)(Error, privsType), privsType privs)
{ return UpdateProfile(ident, &profileSrvr, 1, profile, timeout, async, new CompletionWithContents<Error, privsType>(f, privs, true));}
}; // namespace WONAPI  -- (end of the block above)
