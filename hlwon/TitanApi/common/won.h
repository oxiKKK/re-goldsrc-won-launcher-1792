...
// MSVC 6.0's std::ostream has no operator<< for unsigned __int64 (only the signed
// __int64 overload above), so `os << (unsigned __int64)x` is ambiguous.  Provide it
// for VC6 only; modern toolsets resolve unsigned __int64 natively (an unguarded free
// overload would instead make the modern build ambiguous against the member version).
inline ostream&
operator<<(ostream& os, unsigned __int64 theValue)
{
	char s[21];
	os << _ui64toa(theValue, s, 16);
	return os;
}
#endif
